...
#define DEBUG_TYPE "random-loopsink"

static unsigned SinkFrequencyPercentThreshold = 90;
static unsigned MaxNumberOfUseBBsForSinking = 30;
...
static bool sinkLoopInvariantInstructions(Loop &L, AAResults &AA, LoopInfo &LI,
  ...
  for (Instruction &I : llvm::make_early_inc_range(llvm::reverse(*Preheader))) {
    ...
    if (!canSinkOrHoistInst(I, &AA, &DT, &L, MSSAU, false, LICMFlags))
      continue;
    // NOTE: Random Instrumentation
    if (whether() && sinkInstruction(L, I, ColdLoopBBs, LoopBlockNumber, LI, DT,
                                     BFI, &MSSAU)) {
      Changed = true;
      if (SE)
...
PreservedAnalyses RandomLoopSinkPass::run(Function &F,
