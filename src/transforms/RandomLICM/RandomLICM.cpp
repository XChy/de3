...
static bool ControlFlowHoisting = false;
static bool SingleThread = false;
static uint32_t MaxNumUsesTraversed = 8;
static unsigned FPAssociationUpperLimit = 5U;

extern cl::opt<unsigned> llvm::SetLicmMssaOptCap;

extern cl::opt<unsigned> llvm::SetLicmMssaNoAccForPromotionCap;
...
PreservedAnalyses RandomLICMPass::run(Loop &L, LoopAnalysisManager &AM,
  ...
  LoopInvariantCodeMotion LICM(SetLicmMssaOptCap,
                               SetLicmMssaNoAccForPromotionCap, true);
...
PreservedAnalyses RandomLNICMPass::run(LoopNest &LN, LoopAnalysisManager &AM,
                                       LoopStandardAnalysisResults &AR,
                                       LPMUpdater &) {
  if (!AR.MSSA)
...
bool LoopInvariantCodeMotion::runOnLoop(Loop *L, AAResults *AA, LoopInfo *LI,
  ...
  if (L->hasDedicatedExits())
    Changed |=
        LoopNestMode
            ? UnoptGen::sinkRegionForLoopNest(DT->getNode(L->getHeader()), AA,
                                              LI, DT, TLI, TTI, L, MSSAU,
                                              &SafetyInfo, Flags, ORE)
            : UnoptGen::sinkRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI,
                                   TTI, L, MSSAU, &SafetyInfo, Flags, ORE);
  Flags.setIsSink(false);
  if (Preheader)
    Changed |= UnoptGen::hoistRegion(DT->getNode(L->getHeader()), AA, LI, DT,
                                     AC, TLI, L, MSSAU, SE, &SafetyInfo, Flags,
                                     ORE, LoopNestMode, LicmAllowSpeculation);
    ...
    if (!HasCatchSwitch) {
      ...
      do {
        ...
        for (auto [PointerMustAliases, HasReadsOutsideSet] :
          ...
          LocalPromoted |= UnoptGen::promoteLoopAccessesToScalars(
...
///
bool UnoptGen::sinkRegion(DomTreeNode *N, AAResults *AA, LoopInfo *LI,
...
bool UnoptGen::sinkRegionForLoopNest(
    DomTreeNode *N, AAResults *AA, LoopInfo *LI, DominatorTree *DT,
    TargetLibraryInfo *TLI, TargetTransformInfo *TTI, Loop *CurLoop,
    MemorySSAUpdater &MSSAU, ICFLoopSafetyInfo *SafetyInfo,
  ...
  while (!Worklist.empty()) {
    ...
    Changed |=
        UnoptGen::sinkRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI, TTI,
                             L, MSSAU, SafetyInfo, Flags, ORE, CurLoop);
  }
  return Changed;
}
...
///
bool UnoptGen::hoistRegion(DomTreeNode *N, AAResults *AA, LoopInfo *LI,
                           DominatorTree *DT, AssumptionCache *AC,
...
/// loop invariant.
///
bool UnoptGen::promoteLoopAccessesToScalars(
    const SmallSetVector<Value *, 8> &PointerMustAliases,
    SmallVectorImpl<BasicBlock *> &ExitBlocks,
    SmallVectorImpl<Instruction *> &InsertPts,
    SmallVectorImpl<MemoryAccess *> &MSSAInsertPts, PredIteratorCache &PIC,
...
static bool hoistGEP(Instruction &I, Loop &L, ICFLoopSafetyInfo &SafetyInfo,
  ...
  // FIXME: do some modification to this function
  auto NonNegative = [&](Value *V) { return isKnownNonNegative(V, DL, 0); };
